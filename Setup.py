...
def shift_details():
    ...
    if st.session_state['SETUP_CONFIG_FORM'] == True and st.session_state['FILE_NAME'] == "data_compal.csv":
        ...
        step = datetime.timedelta(minutes=1)
        default_start = datetime.time(7, 0)
...
def state_personalized(device_name, device, col1, col_extra):
    ...
    for i in range(n):
        if n ==3:
            ...
            state_info["max"].append(col3.number_input('Interval 1 maximum:', value=79, step=1, key='max_1_'+device_name))
            state_info["name"].append(col4.text_input('Interval 1 name:', value='Off', key='name_1_'+device_name))
            
            state_info["min"].append(col2.number_input('Interval 2 minimum:', value=80, step=1, key='min_2_'+device_name))
...
def clustering(device):
    ...
    elif "Categorical" not in device["settings"]:
        new = {"column": "state", "variable_type": "Categorical", "variable_name": "State"}
        device["settings"]["Categorical"] = {'State': new}
...
def stop_personalized(device_name, device, col1, col_extra):
    ...
    for i in range(n_stop):
        if n_stop == 2:
            ...
            stop_info["max"].append(col3.number_input('Interval 1 maximum:', min_value=0, value=5, step=1, key='stop_max_1_'+device_name))
            stop_info["name"].append(col4.text_input('Interval 1 name:', value='Micro Stop', key='stop_name_1_'+device_name))
            
            stop_info["min"].append(col2.number_input('Interval 2 minimum:', min_value=0, value=6, step=1, key='stop_min_2_'+device_name))
...
def data_editor(device, key):
    ...
    if settings['variable_type'] == "Empty":
        ...
        st.warning(":arrow_up: The selected column has negative values. Negative values are not supported.")
        st.stop()
...
def process_file():
    ...
    tab1, tab2, tab3 = st.tabs(["Upload :arrow_up:", "Options :gear:", "Data Preview :chart_with_downwards_trend:"])
    error = False
    dict = {}
    with tab1:
...
def metrics_setup(device_name, device):
    
    st.subheader("Metrics")
    
    col1, col2 = st.columns([1, 1])
    
    file = col1.file_uploader("Upload Quality File", type=["csv"], key='metrics_quality_file_'+device_name)
    
    if file is None:
        if st.session_state['SETUP_CONFIG_FORM'] == True and st.session_state['FILE_NAME'] == "data_compal.csv":
            file = "data/compal_quality.csv"
    ...
    options = list(device['settings']['Categorical'].keys())
    product = col2.selectbox("Select Product Variable", options=options, index=1, key='metrics_product_variable_'+device_name)
    baseline = col2.number_input("Expected Production per hour:", min_value=1, value=33500, step=1, key='expected_production_'+device_name)
    
    stop_colum =  device['settings']['Categorical'][device['stop_info']['stop']]
    options = device['data'][stop_colum['column']].unique()
    default = ["Fim de semana", "Falta de Encomenda", "Manutenção Preventiva", "Reunião", "Testes Industriais", "Feriado"]
    scheduled_stops = st.multiselect("Select Schedule Stops", options=options, default=default, key='metrics_scheduled_stops_'+device_name)
    options = list(device['settings']['Categorical'].keys())
    family_products = st.multiselect("Select Family of Products", options=options, default=options[5], key='metrics_family_products_'+device_name)
    
    device['metrics'] = {'quality_data': data, 'product_variable': product, 'baseline': baseline, 'scheduled_stops': scheduled_stops, 'family_products': family_products}
    ...
    for device_name, device in st.session_state.devices_list.items():
        ...
        with st.expander(device_name, expanded=expanded):
            ...
            if device['stop_info']['from_data'] == True:
                device = metrics_setup(device_name, device)
         
    st.toast("Setup completed!", icon='🎉')
    st.toast("Head to the Visualization page to explore your data.", icon='📊')
    st.success("Setup completed! Head to the Visualization page to explore your data.")
