...
class Device:
    def __init__(self, device_name):
        ...
        if self.stop_info['from_data'] == True:
...
def sidebar():
    ...
    with st.sidebar:
        ...
        # if start_date == end_date then allow filter by time
        if device.data[time_column].min().date() == device.data[time_column].max().date():
            start_time = st.time_input("Select a start time", device.data[time_column].min().time(), key="start_time_sidebar")
            end_time = st.time_input("Select an end time", device.data[time_column].max().time(), key="end_time_sidebar")
            if start_time > end_time:
                st.error("Error: End time must fall after start time.")
                st.stop()
            device.data = device.data[(device.data[time_column].dt.time >= start_time) & (device.data[time_column].dt.time <= end_time)]
...
def plot_shift_activity_pie_chart(df_1, df_2, df_3, plot_variables):
    ...
    # text = base.mark_text(radius=80, size=20).encode(text="Count:Q")
    # c = pie + text
...
def plot_shift_activity_normalized_bar_chart(df_1, df_2, df_3, plot_variables):
    ...
    bars = alt.Chart(stacked_df, title="Shift Activity").mark_bar(cornerRadius=15).encode(
    ...
    # text = alt.Chart(stacked_df).mark_text(dx=-5, dy=3, color='white').encode(
    #     x=alt.X('Percentage:Q', scale=alt.Scale(domain=[0, 1]), axis=alt.Axis(format='%')),
    #     y=alt.Y('Time Period', sort=['All time', 'In shift time', 'Outside of shift time']),
    #     detail='Categorical:N',
...
def plot_shift_activity_horizontal_grouped_bar_chart(df_1, df_2, df_3, plot_variables):
    ...
    c = alt.Chart(stacked_df, title="Shift Activity").mark_bar(cornerRadius=15).encode(
        ...
        color= alt.Color('Categorical:N', title=plot_variables['Categorical']['Name']),
...
def visualization():
    ...
    modes = ["Time Series", "Shift Analysis", "Temporal Analysis", "Stop Analysis"]
    if device.stop_info['from_data'] == True:
        modes.append("Metrics")
          
    plots = st.multiselect("Visualization mode:", modes, 
                               default=modes, key=f"plots_{device['name']}")
    ...
    for plot in plots:
        ...
        if plot == "Metrics":
            with st.spinner("Loading metrics ..."):
                st.write("#### Metrics")
                metrics(copy.deepcopy(device), plot=True)
