...
def plot_interruptions(data, stop_info, list_interruptions):
    ...
    base = alt.Chart(merged_df, title="Stops Time & Occurrences").encode(alt.X('day:T', title='Date', axis=alt.Axis(grid=True)), alt.Color('state:N', title='State'))
...
def heat_map(df):
    ...
    for month_year in unique_months_years:
        ...
        # Create a heatmap
        time_chart = alt.Chart(merged_df, title='Stops Total Time').mark_rect().encode(
        ...
        occurrences_chart = alt.Chart(merged_df, title='Stopss Occurrences').mark_rect().encode(
            alt.X('week_day:N', title='', axis=alt.Axis(labelAngle=0, labelAlign='center', orient='top' ), sort=['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']),
            alt.Y('week_number:O', title='', axis=alt.Axis(labels=True), sort='ascending'),
            alt.Color('Interruptions:Q', title='Occurrences'),
            tooltip=[alt.Tooltip('day:T', title='Day'), alt.Tooltip('Interruptions', title='Stops')]
        ).properties(height=400)
...
def plot_interruptions_week_heatmap(stop_info, list_interruptions):
    ...
    time_chart = alt.Chart(df, title='Stops Total Time').mark_rect().encode(
    ...
    occurrences_chart = alt.Chart(df, title='Stops Occurrences').mark_rect().encode(
            alt.X('hour:O', title='', axis=alt.Axis(labelAngle=0)),
            alt.Y('week_day:N', title='', sort=['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']),
...
def plot_pie_chart_stop_distribution(df, data, mode, state_info, settings, stops_dict):
    ...
    if mode == "Based on the number of occurrences of the stops":
    ...
    elif mode == "Based on duration of the stops":
...
# Cannot use st.cache_data
def stop_distribution(df, stop_info, state_info, settings, stops_dict, metrics):
    
    col1, col2 = st.columns([1, 1])
    mode = col1.selectbox("Select mode to calculate percentage:", ["Based on duration of the stops", "Based on the number of occurrences of the stops"], index=0,key="radio_stop_distribution_mode")
    stop_type = col2.selectbox("Select type of stop:", ["All Stops", "Scheduled Stops", "Unscheduled Stops"], index=2 ,key="radio_stop_distribution_stop_type")
    
    
    if stop_type == "Unscheduled Stops":
        df_stops = pd.DataFrame(stops_dict)
        stop_name = stop_info['stop']
        stop_column = settings['Categorical'][stop_name]['column']
        df_stops = df_stops[~df_stops[stop_column].isin(metrics['scheduled_stops'])]
        stops_dict = df_stops.to_dict(orient='records')
        df = df[~df[stop_column].isin(metrics['scheduled_stops'])]
        
    elif stop_type == "Scheduled Stops":
        df_stops = pd.DataFrame(stops_dict)
        stop_name = stop_info['stop']
        stop_column = settings['Categorical'][stop_name]['column']
        df_stops = df_stops[df_stops[stop_column].isin(metrics['scheduled_stops'])]
...
def plot_filtered_line_chart(df, var_name, values, settings):
    ...
    # Count the occurrences of each classification and normalize to get percentages
    classification_count = filtered_df[column_filter].value_counts()
    classification_percentage = (classification_count / total_rows * 100).reset_index()
    classification_percentage.columns = [column_filter, 'percentage']
    
    classification_percentage['constant'] = var_name
    
    chart = alt.Chart(classification_percentage).mark_bar().encode(
        y=alt.Y(field='constant', type='nominal', title=''),
        # limit domain of x to 0-100
        x=alt.X('percentage:Q', title='Percentage (%)', scale=alt.Scale(domain=[0, 100])),
        color = alt.Color(field=column_filter, type='nominal', legend=None),
    ).properties(
        width='container',
    )
    
    # chart = alt.Chart(filtered_df).mark_bar().encode(
    #     x=alt.X('sum(duration)', stack="normalize", axis=alt.Axis(format='%')),
    #     y=alt.Y(field=column_filter, type='nominal'),
    #     color=alt.Color(field=column_filter, type='nominal')
    # )
...
# Cannot use st.cache_data
def stop_histogram(stop_info, settings, stops_dict, stop_type, metrics):
    
    df = pd.DataFrame(stops_dict)
    
    col1, col2, col3 = st.columns([2, 1, 1])
    
    mode = col1.selectbox("Select mode of Histogram:", ["All Stops", "Filtered"], key="selectbox_stop_histogram_mode")
    ...
    if mode == 'Filtered':
        ...
        if stop_type == "From Data":
            stop_type = col2.selectbox("Select type of stop:", ["All Stops", "Scheduled Stops", "Unscheduled Stops"], index=2 ,key="selectbox_stop_distribution_stop_type")
        ...
        filter = col2.multiselect(f"Select {filter_by}:", df[column].unique(), default= df[column].unique() if filter_by == 'State' else df[column].unique(), key="multiselect_stop_histogram_filter")
        plot_filtered_stops_histogram(df, filter_by, filter, settings, scale)
        plot_filtered_line_chart(df, filter_by, filter, settings)
...
def plot_stop_line_bar(grouped_df, column, mode, x_field, type):
    ...
    base = alt.Chart(grouped_df, title="Stops Duration Per Day").encode(
        x = alt.X(field=x_field, type='temporal', title='Date', axis=alt.Axis(grid=True, format='%d/%m/%Y')),
    ...
    if type == "Duration":
        ...
        st.altair_chart(occorrences, use_container_width=True)
    
    return

# Cannot use st.cache_data
def stop_line_bar(settings, stop_info, stops_dict, stop_type, metrics):
    ...
    if stop_type == "From Data":
        stop_type = st.selectbox("Select type of stop:", ["All Stops", "Scheduled Stops", "Unscheduled Stops"], index=2 ,key="selectbox_stop_distribution_stop_typee")
        
        if stop_type == "Unscheduled Stops":
            stop_name = stop_info['stop']
            stop_column = settings['Categorical'][stop_name]['column']
            adjusted_df = adjusted_df[~adjusted_df[stop_column].isin(metrics['scheduled_stops'])]
...
def metrics_old(small_df, big_df, plot_variables, state_info, settings, print_metrics):
    ...
    return run_time, stop_time, availability
...
def overall_metrics(device):
    ...
    metrics_old(small_df, big_df, device.plot_variables, device.state_info, device.settings, True)
...
def stop_activity_from_data(device):
    ...
    col1.write("#### Stops Timeline")
    col2.write("")
    show_stop_timeline = st.toggle("Show Plot", key="toggle_stop_timeline", value=True if device.time_delta().days <= 92 else False)
    if device.time_delta().days > 93:
        if show_stop_timeline==False:
    ...
    st.write("#### Stops Distribution")
    stop_distribution(df, stop_info, state_info, device.settings, stops_dict, device.metrics)
    st.write("#### Stops Histogram")
    stop_histogram(stop_info, device.settings.copy(), stops_dict, "From Data", device.metrics)
    st.write("#### Stops over Day")
    stop_line_bar(device.settings, stop_info, stops_dict, "From Data", device.metrics)
    # st.write("#### Metrics")
...
def stop_activity_personalized(device):
    ...
    new_settings['Categorical']['Stop Type'] = {'column': 'stop_type', 'variable_name': 'Stop Type'}
    ...
    st.write("#### Stops Timeline")
    plot_tick_2_help(merged_df, new_settings, device.plot_variables, 'State', False, device.state_info)
    plot_tick_2_help(merged_df, new_settings, device.plot_variables, 'Stop Type', True, device.state_info)
    ...
    st.write("#### Stops Histogram")    
    stop_histogram(device.stop_info, new_settings, df_stops, "Personalized", None)     
        
        
    st.write("#### Stops over Time")     
    stop_line_bar(new_settings, device.stop_info, df_stops, "Personalized", None)  
    ...
    st.write("#### Stops Temporal Analysis")
...
#data/data_pedreira.xlsx filter=lfs diff=lfs merge=lfs -text
#data/data_pedreira.pkl filter=lfs diff=lfs merge=lfs -text